...
class DirectionTree(object):
    ...
    def generate_tree(self, n=0):
        ...
        if self.pathname.is_dir():
            ...
            # 以上  我添加的
            # self.tree += '    |' * n + '-' * 4 + str(self.pathname.relative_to(self.pathname.parent)) + '\\' + str(newname)+'\n'
            self.tree += '    |' * n + '-' * 4 + \
    ...
    def move(self):
        ...
        if self.pathname.parts[-1][0] == 'c':
            # print(self.pathname)
            files=os.listdir(self.pathname.parent)
            # newfold=str(self.pathname.parent.parent.parent)+"\\single\\"
            # print(newfold)
            if len(files)==1:
                # print(self.pathname.parent.parts)

                newfold=str(self.pathname.parent.parent.parent)+"\\single\\"+self.pathname.parent.parts[-1]+"\\"
                # print(newfold)
                # shutil.copytree(self.pathname.parent,newfold) #复制
                shutil.rmtree(self.pathname.parent,newfold) #删除
                # shutil.move(self.pathname.parent,newfold) #移动
                

            # print(len(files))
            # fold_num = sum([os.path.isfile(listx) for listx in os.listdir(self.pathname.parent  )])
            # print(fold_num)
